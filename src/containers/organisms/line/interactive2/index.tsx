...
import { twoPointPos, vectorDist } from "../../../../utill/calc";
...
export default class SvgIntractSnapMultipleLines extends React.Component<
  Props,
  { tips: tip[] }
> {
  ...
  mousemove() {
    ...
    const next: any[] = data.map(item => {
      ...
      const dist1 = vectorDist(now, a);
      const dist2 = vectorDist(now, b);
          ...
          {data.map(item => (
            <g className="graph" key={item.key}>
              <path
                ...
                d={line(item.values as any) as any}
              />
            </g>
          ))}
        </g>
